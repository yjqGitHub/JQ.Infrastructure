...
namespace JQ.Redis.StackExchangeRedis
{
    ...
    internal sealed class ConnectionMultiplexerFactory
    {
        ...
        {
            LogUtil.Debug($"redis内部发生错误,{e.EndPoint}:{e.Message}", type: typeof(ConnectionMultiplexerFactory));
        }
        ...
        {
            LogUtil.Error($"redis重新连接时发生错误,{e.EndPoint}{e.Exception.ToErrMsg()}", type: typeof(ConnectionMultiplexerFactory));
        }
        ...
        {
            LogUtil.Error($"redis连接时发生错误,{e.EndPoint}{e.Exception.ToErrMsg()}", type: typeof(ConnectionMultiplexerFactory));
        }
